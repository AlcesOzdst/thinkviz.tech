...
/**
 * Pure TypeScript Alpha-Beta Pruning Engine.
...
export function generateAlphaBetaSteps(root: TreeNode): AlgorithmStep<TreeState>[] {
  ...
  // Recursively collect all descendants to mark them as pruned
  ...
  function pushStep(desc: string, currentId: string, line: number, alphaVal: number, betaVal: number) {
    steps.push({
      ...
      metrics: {
        ...
        frontierSize: prunedNodes.size, // Hijack frontierSize to show pruned nodes count
        pathCost: 0,
        totalSteps: 0,
      }
    });
  ...
  function alphaBeta(node: TreeNode, alpha: number, beta: number): number {
    pushStep(`Visiting node ${node.id} (${node.isMaxNode ? 'MAX' : 'MIN'}). α=${alpha === -Infinity ? '-∞' : alpha}, β=${beta === Infinity ? '+∞' : beta}`, node.id, 1, alpha, beta);
    
    // Leaf node check
    if (node.children.length === 0) {
      evaluatedNodes[node.id] = node.value!;
      pushStep(`Leaf node reached. Found terminal value: ${node.value}`, node.id, 2, alpha, beta);
      return node.value!;
    }
    ...
    if (node.isMaxNode) {
      ...
      for (let i = 0; i < node.children.length; i++) {
        ...
        if (beta <= alpha) {
          ...
          if (prunedIds.length > 0) {
             pushStep(`β (${beta}) ≤ α (${alpha}). Pruning ${prunedIds.length} remaining branches!`, node.id, 6, alpha, beta);
          }
          break; // Beta cut-off
        }
      }
      
      pushStep(`MAX node fully evaluated. Best choice is ${maxEval}.`, node.id, 7, alpha, beta);
      return maxEval;
    } else {
      ...
      for (let i = 0; i < node.children.length; i++) {
        ...
        if (beta <= alpha) {
          ...
          if (prunedIds.length > 0) {
             pushStep(`β (${beta}) ≤ α (${alpha}). Pruning ${prunedIds.length} remaining branches!`, node.id, 11, alpha, beta);
          }
          break; // Alpha cut-off
        }
      }
      
      pushStep(`MIN node fully evaluated. Best choice is ${minEval}.`, node.id, 12, alpha, beta);
      return minEval;
    }
  }
  ...
  pushStep(`Alpha-Beta complete. Optimal game value is ${evaluatedNodes[root.id]}.`, root.id, 13, -Infinity, Infinity);
